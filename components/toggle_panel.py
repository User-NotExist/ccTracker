...
class TogglePanel:
    ...
    def _build(self):
        Label(self.frame, text="Toggle Components:", font="Arial 12 bold").pack(side="left", padx=(0, 10))

        Checkbutton(self.frame, text="Tickers", variable=self.show_ticker,
                    command=lambda: self._on_toggle("SHOW_TICKER", self.show_ticker, "ticker")).pack(side="left", padx=5)
        Checkbutton(self.frame, text="Order Book", variable=self.show_order_book,
                    command=lambda: self._on_toggle("SHOW_ORDER_BOOK", self.show_order_book, "order_book")).pack(side="left", padx=5)
        Checkbutton(self.frame, text="Candlestick", variable=self.show_candlestick,
                    command=lambda: self._on_toggle("SHOW_CANDLESTICK", self.show_candlestick, "candlestick")).pack(side="left", padx=5)
        Checkbutton(self.frame, text="Recent Trade", variable=self.show_recent_trade,
                    command=lambda: self._on_toggle("SHOW_RECENT_TRADE", self.show_recent_trade, "recent_trade")).pack(side="left", padx=5)
        Checkbutton(self.frame, text="Best Trade", variable=self.show_best_trade,
                    command=lambda: self._on_toggle("SHOW_BEST_TRADE", self.show_best_trade, "best_trade")).pack(side="left", padx=5)
